...
@input_keywords = ("MIDDLE_WHITELINE_VALUE(integer)","LEFT_WHITELINE_VALUE(integer)","RIGHT_WHITELINE_VALUE(integer)",
		   "BUMPSWITCH_ANY","BUMPSWITCH_1","BUMPSWITCH_2","BUMPSWITCH_3","BUMPSWITCH_4","BUMPSWITCH_5",
		   "FRONT_IR_VALUE(integer)","LEFT_IR_VALUE(integer)","RIGHT_IR_VALUE(integer)","BATTERY_VOLTAGE(integer)","RECEIVE(integer)",
                   "MOTOR_SHAFT_R","MOTOR_SHAFT_L","FIR_LESS_THRE","RIR_LESS_THRE","LIR_LESS_THRE","KINECT(integer)");
...
@output_keywords = ("BUZZER_ON","BUZZER_OFF","MOTOR_LEFT_SPEED(integer)","MOTOR_RIGHT_SPEED(integer)",
		    "MOVE_FWD","MOVE_REV","MOVE_RIGHT","MOVE_LEFT","MOVE_STOP","MOVE_INPLACE_LEFT",
		    ...
		    "MOVE_LEFT_DEGREE(integer)","LCD_INIT","LCD_CLEAR","LCD_DISPLAY_1(string)","LCD_DISPLAY_2(string)",
                    "LCD_DISPLAY_INT_1(integer)","LCD_DISPLAY_INT_2(integer)","SERVO_A_DIRECTION(integer)",
		    "SERVO_B_DIRECTION(integer)","SERVO_C_DIRECTION(integer)","SERVO_TURN_OFF","SET_FIR_THRESHOLD(integer)",
...
@sensor_keywords = ("MIDDLE_WHITELINE_VALUE(integer)","LEFT_WHITELINE_VALUE(integer)","RIGHT_WHITELINE_VALUE(integer)",
		    "FRONT_IR_VALUE(integer)","LEFT_IR_VALUE(integer)","RIGHT_IR_VALUE(integer)","BATTERY_VOLTAGE(integer)","KINECT(integer)");
...
if( $length_output !=0 )
{
 for $elem (@output_signature)
 {
	...
	if ( $elem eq "SERVO_A_DIRECTION(integer)" )
	{
	  print OF "$modulename\_O_SERVO_A_DIRECTION(int val)\n";
 	  print OF "{\n";
	  print OF "	SERVO_A_DIRECTION(val);\n";
	  print OF "}\n";	
	}
	if ( $elem eq "SERVO_B_DIRECTION(integer)" )
	{
	  print OF "$modulename\_O_SERVO_B_DIRECTION(int val)\n";
 	  print OF "{\n";
	  print OF "	SERVO_B_DIRECTION(val);\n";
	  print OF "}\n";	
	}
	if ( $elem eq "SERVO_C_DIRECTION(integer)" )
	{
	  print OF "$modulename\_O_SERVO_C_DIRECTION(int val)\n";
 	  print OF "{\n";
...
if( $length_sensor !=0 )
{
 for $ele (@sensor_signature)
 {
	...
	if ( $ele eq "KINECT(integer)" )
	{
	   print OF "int $modulename\_S_KINECT(void)\n";
	   print OF "{\n";
	   print OF "        return gesture;\n";
	   print OF "}\n";
	}
...
if ( $length_await != 0 )
{
  ...
  for $e133 (@await)
  {
	...
	if ( $e133 eq "KINECT" )
	{
	   ...
	   print OF "  $modulename\_I_KINECT(GESTURE_VALUE);\n";
